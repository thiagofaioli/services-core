class UseIsOwnerOrAdminOnProjcetDetails < ActiveRecord::Migration
  def up
    execute <<-SQL
CREATE OR REPLACE VIEW "1"."project_details" AS 
 SELECT p.id AS project_id,
    ...
    json_build_object('id', u.id, 'name', (CASE WHEN is_owner_or_admin(p.user_id) THEN u.name ELSE u.public_name END), 'public_name', u.public_name) AS "user",
