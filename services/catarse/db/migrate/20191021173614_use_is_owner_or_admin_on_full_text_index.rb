class UseIsOwnerOrAdminOnFullTextIndex < ActiveRecord::Migration
  def up
    execute <<-SQL
CREATE OR REPLACE VIEW "1"."projects" AS 
 SELECT p.id AS project_id,
    ...
    (CASE WHEN is_owner_or_admin(p.user_id) THEN u.name ELSE u.public_name END) AS owner_name,
    c.name AS city_name,
    (CASE WHEN is_owner_or_admin(p.user_id) THEN p.full_text_index ELSE null::tsvector END)::tsvector AS full_text_index,
    is_current_and_online(p.expires_at, (p.state)::text) AS open_for_contributions,
    elapsed_time_json(p.*) AS elapsed_time,
    (COALESCE(pss.score, (0)::double precision))::numeric AS score,
